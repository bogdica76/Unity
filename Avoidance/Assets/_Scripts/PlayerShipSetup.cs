...
public class PlayerShipSetup : MonoBehaviour {
	...
	void Start () {
        int selectedShipIndex = PlayerPrefs.GetInt(Constants.SelectedShipIndex);
        Ship selectedShip = GameObject.Find("ShopManager").GetComponent<ShopManager>().storeShips[selectedShipIndex];
        string shipModel = selectedShip.model;
